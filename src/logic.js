...
// the player always acts first, instead of a rotating dealer
// no blinds have been implemented
...
class TexasHoldemGame {
    ...
    if (!player.folded) {
      ...
      if (action === 'bet') {
        ...
        if (isNaN(betAmount) || betAmount <= 0) {
          console.log("Invalid bet amount. You check by default.");
        } else {
        ...
        console.log("You checked. Bots also check.");
      }
    }
  ...
  async showdown() {
    ...
    const action = await askQuestion("next hand? (y/n)");
    if(action === 'y') {
      this.newHand();
    }
    else return;
